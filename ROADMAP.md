...
`FAQ.md` 2절 16·17번과 1절 12주차에 반영했다.
...
10~13주차 누적본은 **GitHub에 올려야만 검증된다.** 워크플로 문법 오류·Secrets 누락·job 연결
실패는 파일을 아무리 들여다봐도 드러나지 않고, push해서 Run workflow를 눌러야 확인된다.
그렇다고 강사 실제 저장소를 쓰면 실행 이력이 뒤섞여 11주차 수업 자료인 실측표가 오염되고,
Secrets를 지웠다 복구하는 11주차 실습을 검증하다 복구를 잊으면 운영이 조용히 멈춘다.
...
저장소 Streamlit 배포」 절). 이걸 놓쳤다면 12주차에 35명이 동시에 막히고, 그 자리에서 원인을
찾는 최악의 상황이 됐을 것이다.
...
"13주차 누적본이 실제로 도는가"를 확인하려고 만든 것이라 **파일이 원본과 바이트 단위로 같아야**
...
학생용 안내는 `FAQ.md` 2절 20번(트리거 소모)과 1절 각 주차에, 수업 진행 주의는
`CURRICULUM.md` 10·11·13주차에 반영했다.

### 커리큘럼 판단 3건

1. **git 인증(PAT/SSH)을 1주차 → 10주차로 옮겼다.** 커리큘럼은 *"이 주차의 실제 위험은 인증이니
   대면 ③의 대부분을 여기 배정한다"* 였고 `FAQ.md`는 *"9주차까지 GitHub에 올릴 일이 없다"* 여서
   **두 문서가 정면 충돌**하고 있었다. 커리큘럼 쪽을 고쳤다. 결정적 근거는 **PAT의 기본 만료가
   30일**이라는 것 — 1주차에 발급한 토큰은 10주차에 이미 죽어 있어 인증을 두 번 겪게 되고,
   두 번째에는 *"1주차엔 됐는데 왜 또 안 되지"* 라는 더 나쁜 혼란이 얹힌다.
2. **워크플로 YAML은 따라 치지 않고 붙여넣는다**(10·13주차). 실패 지점이 전부 눈에 보이지 않는
   종류이고(CRLF·들여쓰기·보이지 않는 공백), **막히는 원인이 배울 내용과 무관**하다. 아낀 시간은
   YAML을 읽는 데 쓰고 남으면 앞 주차 복습에 쓴다. **파이썬 따라 치기는 그대로 유지**한다.
3. **M5·M10·M12·M16·M18을 확정**했다(상세는 `WEEKLY_BUILD.md` 4절). 전부 "그 주차 학생이 답할 수
   없는 질문을 안고 가게 되는가"를 기준으로 갈렸다.

### 누적본 제작 착수 — 9~13주차 완료 (5/13)

`bigd_py/00_weekly_builds/`(리포 바깥 — 실수로 커밋될 여지를 없앴다)에 **13주차부터 거꾸로**
만들었다(폴더 `bigd_09_module` ~ `bigd_13_trigger`). 전부 컴파일 통과했고 13주차는 실행
검증까지 마쳤다.

**8주차부터는 성격이 바뀐다** — M1 때문에 "빼기"가 아니라 손으로 재구성이다(`stock_utils.py`가
없어지고 함수들이 `notify_stock_price.py` 안으로 들어간다).

**제작 중 얻은 교훈**: 복사로 누적본을 만들면 **그 주차에 없어야 할 파일이 딸려 온다.** 실제로
`README.md`가 9~12주차 폴더에 딸려 갔다가 뒤늦게 발견해 지웠다. 실행 검증으로는 잡히지 않는 유형이라
