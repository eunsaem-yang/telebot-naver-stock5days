...
이 프로젝트의 **설계 판단과 그 판단이 딛고 선 전제**를 시간 순으로 남기는 문서다. 처음에는
`notify_stock_price.py`에 추가하고 싶은 기능과 구현 방법 후보를 적는 계획서로 출발했지만,
아래 **기능 1~5는 전부 구현이 끝났고**(지금 동작하는 코드의 설명은 `CLAUDE.md` 참고) 그 뒤로는
구현하며 겪은 이슈·재판단·운영 기록이 이어진다. 결론만 적지 않고 **그 결론이 딛고 선 전제를
함께 적는 것**이 이 문서의 원칙이다 — 전제가 바뀌었는지 나중에 확인할 수 있어야 결정을 다시
열어볼 수 있기 때문이다(아래 "N1 재판단" 절의 교훈).

옛 절도 지우지 않고 그대로 둔다. 결론이 뒤집힌 곳은 **원문을 남기고 그 아래에 정정을 덧붙이는**
방식으로 기록한다.

## 출발점의 제약사항 (2026-07 초 시점 — 지금은 해당 없음)

> **주의**: 아래 두 항목은 **이 문서를 처음 쓸 때의 상태**이고 지금은 성립하지 않는다.
> 공공데이터포털 API는 이후 `basDd` 날짜 필터가 아예 동작하지 않는 것이 확인돼 **완전히
> 걷어냈고**(아래 "data.go.kr 포기, 네이버로 통합" 절), 현재는 현재가 조회와 종가 기록을 모두
> **네이버 API 하나**로 처리한다. 같은 이유로 이어지는 기능 1·2의 "구현 스케치"에 남아 있는
> `basDd` fallback 서술도 **실제 구현과 다르다** — 당시의 사고 과정을 남기려고 지우지 않는다.

- 당시 쓰던 공공데이터포털 금융위원회 API(`GetStockSecuritiesInfoService`)는 **일별 종가(EOD) 데이터**만 제공한다.
...
### 후속: 휴장일에 버튼을 누르면 안내 메시지를 보낸다 (2026-08-02)

아래 "DB 적용 전 최종 점검 → 정리한 것"에서 `check_manual_trigger.py`에 `is_trading_day()`
가드를 넣어, 휴장일에 버튼을 눌러도 무거운 `notify` job이 켜지지 않게 했다. 그런데 그 가드에
걸리면 **사용자 쪽에서는 아무 일도 일어나지 않는다.** 버튼을 누른 것은 사용자의 명시적 요청인데
응답이 전혀 없으므로, **"오늘은 휴장일이라 보낼 게 없다"(정상 종료)와 "봇이나 워크플로가
고장났다"가 구별되지 않는다.** P16·P17에서 "Actions 녹색 체크와 실제 장애가 같은 신호를 낸다"를
문제로 본 것과 같은 종류의 모호함이고, 다만 이번엔 그 신호를 받는 쪽이 강사가 아니라 사용자다.

그래서 트리거가 감지됐는데 `is_trading_day()`가 False면, `notify` job을 켜는 대신 **`check`
job이 `send_telegram_message()`로 안내 한 건을 직접 보낸다.**

**왜 여기서만 보내는가 (= 왜 `notify_stock_price.py`에는 안 붙였는가)**: 같은 휴장일 가드가
`notify_stock_price.py`에도 있지만 거기엔 **일부러 안 붙였다.** 그쪽은 하루 3회 자동으로 도는
경로라, 아무도 요청하지 않았는데 공휴일마다 안내가 세 번씩 오게 된다. **판단 기준은 "사용자가
요청했는가"**다. 이것이 보류해 둔 **P11**(자동 실행이 전부 실패했을 때 텔레그램으로 알리는 안)
과의 결정적 차이이기도 하다 — P11을 미룬 이유는 네이버가 장기 차단되면 실패 알림이 자동 스케줄을
따라 계속 푸시된다는 점이었는데, 이번 것은 **누른 만큼만 오므로 빈도를 사용자가 직접 통제한다.**
즉 P11을 여는 것이 아니라, P11이 걸렸던 지점(빈도를 통제할 수 없음)이 없는 경우를 하나 찾은 것이다.

**구조 영향 없음**: 가벼운 `check` job은 이미 `requests`를 설치하고 텔레그램 Secrets를 갖고 있어
**추가 의존성이 0이다.** 동작도 `notify` job은 여전히 `skipped`인 채 텔레그램만 오는 형태라,
"트리거가 없거나 휴장일이면 무거운 job을 안 켠다"는 원래 이득을 그대로 유지한 채 침묵만 없앴다.

**검토했다 기각한 것**: *"휴장일에도 DB에 쌓인 과거 종가로 그래프를 그려 보내면 되지 않나."*
기각 근거 셋:

1. `notify_stock_price.py`의 휴장일 가드를 **호출 경로(자동 스케줄이냐 수동 트리거냐)에 따라
   분기**시켜야 해서, 그 경로를 알려줄 환경변수가 하나 새로 들어온다.
2. 휴장일에는 데이터가 변하지 않으므로 **몇 번을 눌러도 매번 똑같은 그림**이 온다.
3. **무엇보다 대시보드가 이미 그 일을 더 잘 한다** — 요일과 무관하고, 4종목을 한 화면에 보여주고,
   폴링을 기다릴 필요 없이 즉시 열린다.

대신 안내 메시지가 **대시보드를 가리키게** 해서, 코드 0줄을 더 쓰지 않고 그 효용의 대부분을 얻었다.

**여기서 뽑아낸 판단 기준**: 「**새 시세는 push, 쌓인 데이터는 pull**」. 이번 결정의 실질은
"휴장일 안내 문구를 어디에 넣느냐"가 아니라 **어떤 기능을 push와 pull 중 어느 경로에 넣을
것인가**였고, 이 한 줄이 그 답이었다. 같은 질문이 앞으로도 반복될 것이라 `CLAUDE.md`에 판단
기준으로 남겼다.

**실동작 검증**: 일요일(휴장일)에 아래 "테스트 저장소 답사" 절의 테스트 저장소에서 실제로 버튼을
눌러 안내 메시지가 오는 것까지 확인했다.
...
그런데 `CURRICULUM.md`를 다시 보니 **7·8·11·12장이 전부 `price_history.json`을 중심 소재로
...
- 휴장일에 버튼을 눌러도 무거운 `notify` job이 켜지던 것을 막기 위해, `check_manual_trigger.py`가
  `is_trading_day()`까지 확인하고 나서 트리거 여부를 보고하도록 변경. (**2026-08-02 보완**: 이
  분기에서 그냥 조용히 끝내지 않고 **안내 메시지 1건을 텔레그램으로 보낸다** — 버튼을 눌렀는데
  아무 응답이 없으면 정상 종료와 고장을 사용자가 구별할 수 없기 때문이다. 위 "기능 4 → 후속:
  휴장일에 버튼을 누르면 안내 메시지를 보낸다" 절 참고.)
...
목표 서술("텔레그램 봇 API로 전송")과 `CURRICULUM.md`의 관련 장도 함께 재검토가 필요하다.
...
### Streamlit Cloud 배포 재시도: private 저장소 접근 404 → public 전환으로 해결 (2026-07-22) — ⚠️ 진단은 2026-08-02에 정정됨
...
#### 정정 (2026-08-02): private 저장소도 그대로 배포된다 — 승인 버튼이 배포 화면에 없었을 뿐

위 서술은 **"private 저장소는 Streamlit Cloud가 접근할 수 없으므로 public 전환이 해결책"**으로
읽히는데, 이는 틀렸다. 아래 "테스트 저장소 답사" 절에서 만든 **private 저장소
`eunsaem-yang/telebot-stock-test`를 private인 채로 배포하는 데 성공**했다. 07-22의 public 전환은
Actions 분당 한도까지 함께 푸는 선택이었으므로 그 자체로는 여전히 합리적이었지만, **"private면
안 된다"는 것은 사실이 아니다.**

**증상** (07-22와 같은 계열이지만 화면이 셋으로 나뉘어 나타난다):

- New app 화면의 Repository 드롭다운에 **public 저장소만** 나오고 private 저장소는 목록에 없다.
- 저장소 이름을 직접 입력하면 **`This repository does not exist`**.
- Branch·Main file path를 다 채워도 **Deploy 버튼이 계속 비활성**이다.

**함정의 핵심**: 화면이 *"권한이 없다"*가 아니라 ***"저장소가 존재하지 않는다"***고 말한다. 그래서
이름 오타, 대소문자, 업로드 실패, 브랜치 이름 같은 **엉뚱한 곳**을 계속 의심하게 된다. 07-22에
`404 (Not Found)` 응답을 보고 "권한이 없어서 저장소를 못 찾는 것처럼 보인다"고까지 적어놓고도
해결책을 public 전환으로 잡은 것도 같은 이유다.

**해결**: 승인 버튼은 **배포 화면이 아니라 계정 설정에 있다.**

1. 오른쪽 위 프로필 아이콘 → **Settings**
2. **Linked accounts** → GitHub 항목의 **`Connect`**
3. GitHub 인증 화면에서 승인 (private 저장소 접근 권한을 여기서 준다)
4. **배포 화면으로 돌아와 새로고침** — 이 단계를 빼먹으면 승인을 마쳐도 드롭다운에 계속 안 나와서
   "승인해도 안 되네"로 잘못 결론 내리게 된다.

**함께 발견한 것 둘** (배포 자체와는 별개지만 같은 자리에서 걸린다):

- **① TOML 스마트 따옴표**: Secrets 입력란은 TOML 형식인데, 값을 다른 앱(메모장·문서 편집기·
  메신저 등)을 거쳐 붙여넣으면 곧은 따옴표 `"`가 둥근 따옴표 `“ ”`로 바뀌어 있는 경우가 있다.
  이때 **오류 메시지가 전혀 뜨지 않고 Save 버튼만 비활성 상태로 남는다.** 눌리지 않는 버튼을
  계속 누르며 원인을 못 찾게 되는 유형이다.
- **② Secrets를 나중에 넣으면 Reboot가 필요하다**: `stock_utils.py`는 **모듈 import 시점에**
  환경변수를 읽는데, Streamlit의 rerun은 **이미 import된 모듈을 다시 읽지 않는다.** 그래서
  Deploy(Secrets 없이) → Secrets 저장 → rerun 순서로 진행하면 값이 계속 `None`으로 남고,
  **Manage app → Reboot app**을 해야 비로소 반영된다. 진단 신호는 **화면에 에러가 안 뜬다는
  것**이다 — `load_price_history()`가 예외를 삼키고 `{}`를 돌려주므로 대시보드는 멀쩡히 그려지고,
  추이 문구가 `현재가 (종가 히스토리 누적 전)`, x축이 `현재` 하나로 나오는 것으로만 알아챌 수
  있다. 로그(Manage app)에는 `❌ Turso DB에서 종가 히스토리를 읽지 못했습니다: …`가 남는다.
  07-31 토큰 교체 검증 때 "왼쪽이 비었는지로만 판별된다"고 적었던 것과 **정확히 같은 성질**이다.

**셋의 공통점**: 전부 **에러를 내지 않고 조용히 막는 유형**이다(목록에서 사라짐 / 버튼만 비활성 /
빈 그래프). 이 리포가 P16·P17에서 "녹색 체크가 잘못된 신호를 준다"를 문제로 봤던 것과 같은
구조이고, 학생 70명이 각자 배포할 때 가장 오래 붙잡을 자리이기도 하다. 그래서 학생 안내는
`FAQ.md` 2절 16·17번과 1절 12장에 반영했다.
...
**4장(API 호출 실습)부터 13장까지는 전부 API가 실제로 응답한다는 전제 위에 실습이
설계돼 있다.** 학기 중 4장 이후 아무 시점에나 네이버가 막히면 그날 실습이 그대로 막힌다.

### 검토한 대응 방향: 공공데이터포털로 대체 프로젝트를 만들어두는 방식 (기각)

공공데이터포털 API는 EOD(일별 종가)만 제공해 실시간가를 애초에 대체할 수 없다 — 4·6·8주
(실시간/현재가/분봉 관련 장)을 못 살린다. `basDd` 필터 무시 문제(위 "알려진 이슈: 최근
...
3. 수업 당일 네이버가 막히면 토글만 켜고 그대로 진행 — 학생들은 여전히 진짜 JSON 구조·진짜
   그래프를 보고 배우고, 4장 이후 커리큘럼이 밀리지 않는다.
...
- **수동 트리거 조기 ack**: `check_manual_trigger.py`가 알림을 실제로 보내기 **전에**
  ...
  **(2026-08-02 조건 한정)** 이 항목이 지목한 "눌렀는데 아무 일도 안 일어난 것으로 보인다"는
  증상은 **휴장일 경로에 한해 해소됐다** — 그 분기에서는 `check` job이 안내 메시지를 직접
  보내므로 침묵이 남지 않는다(위 "기능 4 → 후속: 휴장일에 …" 절). 다만 **`notify` job이
  켜졌다가 실패하는 경로에서는 이 항목이 그대로 유효하다.** 두 경로의 차이는 실패를 아는
  주체다 — 휴장일은 `check` job이 **자기가 안 보낸다는 사실을 알고 있어서** 그 자리에서
  안내할 수 있지만, `notify` job의 실패는 이미 ack를 끝내고 종료한 `check` job이 알 방법이
  없다. 그래서 후자는 여전히 "전송 성공 후 ack" 구조를 요구하고, 비용 판단도 그대로다.
...
수업 커리큘럼 8장(DB 세팅)를 설계하다가, **학생 다수가 Windows인데 Turso CLI 경로가 한 번도
...
Turso 세팅보다 큰 작업이라, 8장 전체가 무너질 수 있는 위험이었다.
...
8장 세팅이 **①웹 콘솔 로그인 → ②Create Database → ③URL·토큰 복사 → ④`.env`에 두 줄**로 줄어든다.
...
남겨두지 않는다.** (2) 녹화·화면 공유 시 토큰이 보이는 구간을 아예 만들지 않는다(8장 촬영은
...
## 테스트 저장소 답사와 누적본 제작 착수 (2026-08-01~02)

### 왜 저장소를 하나 더 만들었나 — 목적의 절반은 검증이 아니었다

10~13장 누적본은 **GitHub에 올려야만 검증된다.** 워크플로 문법 오류·Secrets 누락·job 연결
실패는 파일을 아무리 들여다봐도 드러나지 않고, push해서 Run workflow를 눌러야 확인된다.
그렇다고 강사 실제 저장소를 쓰면 실행 이력이 뒤섞여 11장 수업 자료인 실측표가 오염되고,
Secrets를 지웠다 복구하는 11장 실습을 검증하다 복구를 잊으면 운영이 조용히 멈춘다.

그래서 `eunsaem-yang/telebot-stock-test`(**private**) + 새 텔레그램 봇 + 새 Turso DB를 만들었다.
**새 계정은 하나도 만들지 않았다** — 격리는 저장소 단위로 이미 완결되고(워크플로·Secrets·실행
이력이 전부 저장소에 매여 있다), 계정을 나누면 로그인 전환 비용만 생긴다.

**목적의 나머지 절반은 "강사가 학생 경로를 먼저 밟아보는 것"이었고, 실제로 그쪽에서 더 많이
건졌다.** 아래 함정 목록이 전부 그 부산물이다.

### private으로 만든 것이 결정적이었다

**더 까다로운 쪽으로 테스트해야 한다 — private에서 되면 public에서도 되지만 반대는 성립하지
않는다.** 실제로 Streamlit 배포에서 public이었으면 존재조차 몰랐을 단계를 만났다(위 「private
저장소 Streamlit 배포」 절). 이걸 놓쳤다면 12장에 35명이 동시에 막히고, 그 자리에서 원인을
찾는 최악의 상황이 됐을 것이다.

### `collect_close.yml` — N1과 P18이 Actions 환경에서 처음으로 함께 검증됐다

**토요일 23:21에 실행했는데 저장된 날짜는 `20260731`(금)** 이었고 4/4종목 성공했다.

- 실행 시각이 아니라 체결 시각(`localTradedAt`)에서 날짜를 뽑으므로 **자정을 넘겨도 날짜가
  밀리지 않는다**(N1 수정). 지금까지는 로컬 실행으로만 확인했었다.
- `is_trading_day()` 가드를 제거했기 때문에 **휴장일에 실행돼도 스킵되지 않는다**(P18 수정).
  가드가 남아 있었다면 이 실행은 통째로 건너뛰어졌을 것이다.

**두 수정이 서로 맞물려야 성립한다는 것이 이 실행 하나로 확인됐다.** 하필 휴장일에 돌린 것이
오히려 운이 좋았다.

### `check_manual_trigger.yml`은 Disable해 두었다 — cron을 고치지 않은 이유

private 저장소에서 이 워크플로가 그대로 돌면 하루 약 120회 × 월 22일 ≈ **2,600분**으로 무료
한도(2,000분)를 넘는다. 실제 리포는 public이라 무제한 무료였기에 지금껏 문제가 없었다.

**cron을 좁히는 대신 워크플로를 Disable했다.** 근거는 검증의 성립 조건이다 — 이 폴더는
"13장 누적본이 실제로 도는가"를 확인하려고 만든 것이라 **파일이 원본과 바이트 단위로 같아야**
한다. cron을 고치면 정작 다른 파일을 검증하게 되고, 누적본 확정 시 되돌리는 것을 잊으면 학생에게
창이 좁혀진 파일이 나간다. Disable은 파일을 한 글자도 바꾸지 않고 실행만 멈춘다.

### 답사에서 나온 함정들 — 전부 "에러를 안 내고 조용히 막는" 유형이다

이것이 이번 답사의 가장 큰 수확이다. 학생이 그대로 겪을 것들이고, 공통점은 **실패했다는 신호가
어디에도 뜨지 않는다**는 점이다.

| 함정 | 겉으로 보이는 것 |
|---|---|
| macOS Finder·Windows 탐색기는 **점으로 시작하는 폴더를 못 만든다**(`.github`) | 이름을 거부당함 |
| **`git add .`의 `.`은 저장소 전체가 아니라 현재 폴더**다 | 커밋·push가 정상 성공하는데 GitHub에 파일이 없다 |
| zsh에서 **`#`은 주석**이라 `git commit -m #"메시지"`가 `git commit -m`이 된다 | 앞서 본 것과 똑같은 오류가 반복 |
| **`getUpdates`의 트리거는 실행이 한 번 지나가면 소모된다**(로컬이든 Actions든) | `🔍 새 텔레그램 메시지가 없습니다.` |
| 편집기 경로에 따라 **줄바꿈이 CRLF로** 들어간다 | 내용은 같은데 원본과 바이트가 다름 |

네 번째는 **강사가 직접 밟았다** — `/notify`를 한 번 누른 뒤 코드를 고치고 Run workflow를 세 번
돌렸는데 전부 놓쳤다. 규칙은 **"한 번 누르면 한 번만 감지된다, 누르는 것이 먼저·실행이 나중"**
이다. 다섯 번째(CRLF)는 워크플로 YAML을 **따라 치지 않고 붙여넣기로 바꾸는 근거**가 됐다.

학생용 안내는 `FAQ.md` 2절 20번(트리거 소모)과 1절 각 장에, 수업 진행 주의는
`CURRICULUM.md` 10·11·13장에 반영했다.

### 커리큘럼 판단 3건

1. **git 인증(PAT/SSH)을 1장 → 10장으로 옮겼다.** 커리큘럼은 *"이 장의 실제 위험은 인증이니
   대면 ③의 대부분을 여기 배정한다"* 였고 `FAQ.md`는 *"9장까지 GitHub에 올릴 일이 없다"* 여서
   **두 문서가 정면 충돌**하고 있었다. 커리큘럼 쪽을 고쳤다. 결정적 근거는 **PAT의 기본 만료가
   30일**이라는 것 — 1장에 발급한 토큰은 10장에 이미 죽어 있어 인증을 두 번 겪게 되고,
   두 번째에는 *"1장엔 됐는데 왜 또 안 되지"* 라는 더 나쁜 혼란이 얹힌다.
2. **워크플로 YAML은 따라 치지 않고 붙여넣는다**(10·13장). 실패 지점이 전부 눈에 보이지 않는
   종류이고(CRLF·들여쓰기·보이지 않는 공백), **막히는 원인이 배울 내용과 무관**하다. 아낀 시간은
   YAML을 읽는 데 쓰고 남으면 앞 장 복습에 쓴다. **파이썬 따라 치기는 그대로 유지**한다.
3. **M5·M10·M12·M16·M18을 확정**했다(상세는 `WEEKLY_BUILD.md` 4절). 전부 "그 장 학생이 답할 수
   없는 질문을 안고 가게 되는가"를 기준으로 갈렸다.

### 누적본 제작 착수 — 9~13장 완료 (5/13)

`bigd_py/00_weekly_builds/`(리포 바깥 — 실수로 커밋될 여지를 없앴다)에 **13장부터 거꾸로**
만들었다(폴더 `bigd_09_module` ~ `bigd_13_trigger`). 전부 컴파일 통과했고 13장은 실행
검증까지 마쳤다.

**8장부터는 성격이 바뀐다** — M1 때문에 "빼기"가 아니라 손으로 재구성이다(`stock_utils.py`가
없어지고 함수들이 `notify_stock_price.py` 안으로 들어간다).

**제작 중 얻은 교훈**: 복사로 누적본을 만들면 **그 장에 없어야 할 파일이 딸려 온다.** 실제로
`README.md`가 9~12장 폴더에 딸려 갔다가 뒤늦게 발견해 지웠다. 실행 검증으로는 잡히지 않는 유형이라
(파일이 하나 더 있어도 스크립트는 잘 돈다), 폴더마다 `ls -a` 목록을 구성표와 대조하는 단계를
실행 검증 **앞에** 두기로 했다.
