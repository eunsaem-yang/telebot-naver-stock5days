...
### 누적본 제작 착수 — week09~13 완료 (5/13)

`bigd_py/00_weekly_builds/`(리포 바깥 — 실수로 커밋될 여지를 없앴다)에 **13주차부터 거꾸로**
만들었다. 전부 컴파일 통과했고 week13은 실행 검증까지 마쳤다.

**week08부터는 성격이 바뀐다** — M1 때문에 "빼기"가 아니라 손으로 재구성이다(`stock_utils.py`가
없어지고 함수들이 `notify_stock_price.py` 안으로 들어간다).

**제작 중 얻은 교훈**: 복사로 누적본을 만들면 **그 주차에 없어야 할 파일이 딸려 온다.** 실제로
`README.md`가 week09~12에 딸려 갔다가 뒤늦게 발견해 지웠다. 실행 검증으로는 잡히지 않는 유형이라
