...
```
python setup_telegram_button.py      # macOS: python3 setup_telegram_button.py
```

실행하면 두 가지가 한 번에 준비됩니다.

1. 텔레그램 채팅창 아래에 **`📊 지금 현재가 확인` 리플라이 키보드 버튼**이 뜹니다.
2. 채팅 입력창 옆 **고정 메뉴에 `/notify` 명령어**가 등록됩니다.

같은 일을 하는 경로를 굳이 두 개 두는 이유는, **버튼이 붙어 있는 메시지를 지우면 버튼도 같이
사라지기** 때문입니다. `/notify`는 봇 자체에 등록되는 것이라 메시지를 아무리 지워도 남아 있는
보험 경로입니다.
...
배포 절차 전체는 `FAQ.md` 1절 **12주차**, 막혔을 때는 `FAQ.md` 2절 **17번**을 보세요.
...
여기서 해결되지 않으면 **`FAQ.md`**를 보세요. 1절은 **주차별 세팅**(무엇을 언제 설치·발급하는지),
