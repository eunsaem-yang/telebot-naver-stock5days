---
name: get-chat-id
description: 새 텔레그램 봇 토큰의 TELEGRAM_CHAT_ID를 알아낼 때 사용. telebot.py를 실행해 getUpdates 응답을 조회하고 결과를 해석해준다.
---
...
3. `python ./telebot.py`를 실행한다. 이 스크립트는 `getUpdates` API를 호출해 raw JSON을 출력한다.
