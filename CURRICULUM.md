...
- **실습 지침: "수동 실행"으로 감지→전송 확인하기** — 이 주차의 검증 방법이다. 원래 운영
  ...
  **대조 실험(권장)**: 1번(텔레그램 버튼)을 **누르지 않고** 2번만 실행시켜 본다.
  `check` job은 `🔍 수동 트리거 메시지는 없었습니다.`로 끝나고 `notify` job은 **skipped**로
  건너뛴다. `if: needs.check.outputs.triggered == 'true'`가 실제로 job을 막는 것을 눈으로 확인하는
  실습이라, 두 번 돌려 비교하는 것이 이 주차의 핵심이다.
