...
| **8** | **관계형 DB 기초 & Turso 설계** | **12주 → 8주** |
| **9** | **Turso 연동 + 종가 수집(`collect_daily_close.py`)** | **13주 → 9주** |
| 10 | 시각화 + `sendPhoto` | 8주 → 10주 **← 완결된 프로그램 ②** |
...
1. **DB를 8~9주로 앞당김.** JSON 단계를 없애면 학생이 만드는 것과 저장소가 계속 일치한다.
   10주차 그래프도 이미 쌓인 실제 히스토리로 그리게 되어 실습이 더 실감난다.
...
잃는 것은 **"파일 기반 저장의 한계를 몸으로 겪는 경험"** 하나다.

보완은 cron-job.org를 다루기로 한 방식과 동일하게 — **실습에서 빼고 강사가 사례로만 전달**한다.
8주차에 DB를 도입하면서 15분 정도 이렇게 들려주면 된다:
